...
By default, **Fathom-REST** will serve all requests to `/`.  You may specify the basepath in your `conf/default.conf` file.
...
If you are using annotated *controller* discovery then you are at the mercy of the JVM for registration order of your *controller* methods.

If you need deterministic registration orders you may specify the `@Order(n)` annotation on some or all *controller* methods.  Lower numbered methods are registered first.
